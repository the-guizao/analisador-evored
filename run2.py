...
def gerar_pdf_para_alunos(arquivo_excel, logo_path):
    ...
    for aluno in alunos_unicos:
        ...
        # Adicionar a logo ao canto superior direito
        ...
        # Gerar gráfico da evolução das notas gerais (eixo X com contagem)
        ...
        plt.savefig(f'grafico_geral_{aluno}.png')
        plt.close()

        # Adicionar o gráfico de nota geral ao PDF
        pdf.image(f'grafico_geral_{aluno}.png', x=10, y=30, w=180)
        
        # Adicionar tabela de notas ao PDF
        pdf.ln(90)  # Espaço antes da tabela
        ...
        pdf.cell(40, 10, '', 1)  # Célula vazia para o primeiro "Simulado"
        pdf.cell(30, 10, '', 1)
        pdf.cell(30, 10, '', 1)
        pdf.cell(30, 10, '', 1)
        pdf.cell(30, 10, '', 1)
        ...
        for i in range(len(simulados)):
            ...
            pdf.cell(40, 10, f'Simulado {i+1}: {simulados[i]}', 1)
            pdf.cell(150, 10, f'Nota Geral: {notas_gerais[i]}', 1)
            pdf.ln()
            # Adicionando as competências abaixo do título do simulado
            pdf.cell(40, 10, 'Competências:', 1)
            for nota_comp in competencias[i]:
                pdf.cell(30, 10, str(nota_comp), 1)
            pdf.ln()

        # Gerar gráficos para cada competência
        for comp_idx, comp_label in enumerate(competencias_labels):
            ...
            pdf.ln(20)  # Nova página para cada competência  # Adicionar logo na nova página também
